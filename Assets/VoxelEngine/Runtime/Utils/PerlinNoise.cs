...
namespace VoxelEngine
{
    public static class PerlinNoise
    {
        public static float Perlin3D(float x, float y, float z)
        {
            ...
            float x1, x2, y1, y2;
            x1 = math.lerp(Grad(aaa, xf, yf, zf), Grad(baa, xf - 1, yf, zf), u);
            x2 = math.lerp(Grad(aba, xf, yf - 1, zf), Grad(bba, xf - 1, yf - 1, zf), u);
            y1 = math.lerp(x1, x2, v);

            x1 = math.lerp(Grad(aab, xf, yf, zf - 1), Grad(bab, xf - 1, yf, zf - 1), u);
            x2 = math.lerp(Grad(abb, xf, yf - 1, zf - 1), Grad(bbb, xf - 1, yf - 1, zf - 1), u);
            y2 = math.lerp(x1, x2, v);

            return (math.lerp(y1, y2, w) + 1) / 2; // Normalized to [0, 1]
        }
        ...
        {
            151, 160, 137, 91, 90, 15, // Start of the original 256 values
            ...
            // Repeat the first 256 values to avoid wrapping
